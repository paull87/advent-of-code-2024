...
def check_pattern(grid):
    # if any([any([(0 if w == "." else w) > 1 for w in h]) for h in grid]):
    #     return False
    # return True
    if any([sum([0 if w == "." else w for w in h]) > 10 for h in grid]):
        return True
    return False
...
def main():
    ...
    while True:
        ...
        cur_safety = calculate_safety_factor(grid)
        if cur_safety < safety:
            safety = cur_safety
            visualise_grid(grid)
        print("Secs passed:", i * moves, safety)
        # time.sleep(1)
        i += 1
        moves = 1
